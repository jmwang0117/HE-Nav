...
class PcPreprocessor(nn.Module):
    ...
    def forward(self, pc, indicator):
        ...
        # print("indicator_t shape:", indicator_t.shape)
        # print("indicator_t:", indicator_t)

        for scale in self.pooling_scales:
            xidx, xres = quantitize(pc[:, 0], self.lims[0][0],
                                              self.lims[0][1], torch.div(self.sizes[0], scale, rounding_mode='floor'), with_res=True)
            yidx, yres = quantitize(pc[:, 1], self.lims[1][0],
                                              self.lims[1][1], torch.div(self.sizes[1], scale, rounding_mode='floor'), with_res=True)
            zidx, zres = quantitize(pc[:, 2], self.lims[2][0],
                                              self.lims[2][1], torch.div(self.sizes[2], scale, rounding_mode='floor'), with_res=True)
        ...
        # print("bxyz_indx shape:", bxyz_indx.shape)
        # print("bxyz_indx:", bxyz_indx)
        # print("xyz_res shape:", xyz_res.shape)
        # print("xyz_res:", xyz_res)
