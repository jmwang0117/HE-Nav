...
class LocalAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        B, C, H, W = x.size() # 10 96 8 8
       
        r1, r2 = H // self.window_size[0], W // self.window_size[1]
        
        x_total = einops.rearrange(x, 'b c (r1 h1) (r2 w1) -> b (r1 r2) (h1 w1) c', h1=self.window_size[0], w1=self.window_size[1]) # B x Nr x Ws x C r1=2 r2 =2
        
        x_total = einops.rearrange(x_total, 'b m n c -> (b m) n c') # torch.Size([10, 4, 16, 96])
...
class DAT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, num_classes=1000, expansion=4,
        ...
        self.patch_proj = nn.Sequential(
            nn.Conv2d(128, dim_stem, 7, patch_size, 3),
            LayerNormProxy(dim_stem)
        ) if use_conv_patches else nn.Sequential(
            nn.Conv2d(128, dim_stem, patch_size, patch_size, 0),
            LayerNormProxy(dim_stem)
        ) 
        ...
        self.cls_head = nn.Conv2d(768, num_classes, kernel_size=1)
