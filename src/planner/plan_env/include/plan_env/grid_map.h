...
public:
  ...
  void publishUnknown();
  void publishDepth();
  void OCNetQuery(int x, int y, int z, bool is_occupied); //AGR
  void OccRemappingCallback(const std_msgs::Float64MultiArray::ConstPtr& msg); //AGR
  std::set<int> subscribed_occupied_addresses; //AGR
  void SegmentGroundPoints(); //AGR
  ros::Subscriber occ_update_coords_; //AGR
