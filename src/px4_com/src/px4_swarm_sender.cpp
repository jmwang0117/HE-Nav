...
{
    ros::init(argc, argv, "px4_swarm_sender");
    ros::NodeHandle nh("~");
    ...
    while(ros::ok())
    {
        ...
        case Disarm:
            if(pos_sender.current_state.armed)
            {
                ...
                if(pos_sender.arming_client.call(pos_sender.arm_cmd) && pos_sender.arm_cmd.response.success)
                {
        ...
        case Arm:
            ...
            if(pos_sender.current_state.mode != "OFFBOARD" && (ros::Time::now() - last_request > ros::Duration(5.0)))
            {
                pos_sender.mode_cmd.request.custom_mode = "OFFBOARD";
                if(pos_sender.set_mode_client.call(pos_sender.mode_cmd) && pos_sender.mode_cmd.response.success)
                {
            ...
            {
                if(!pos_sender.current_state.armed && (ros::Time::now() - last_request > ros::Duration(5.0)))
                {
